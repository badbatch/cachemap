import { core, map } from "@cachemap/types";
import { isNumber, isPlainObject } from "lodash";

export class MapStore implements core.Store {
  public static async init(options: map.InitOptions): Promise<MapStore> {
  ...
  constructor(options: map.ConstructorOptions) {
...
export default function init(options: map.Options = {}): core.StoreInit {
  ...
  return (storeOptions: core.StoreOptions) => MapStore.init({ ...options, ...storeOptions });
}
