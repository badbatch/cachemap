import { core, redis } from "@cachemap/types";
import * as fakeRedis from "fakeredis";
import { isNumber, isPlainObject } from "lodash";
import { createClient, RedisClient } from "redis";

export class RedisStore implements core.Store {
  public static async init(options: redis.InitOptions): Promise<RedisStore> {
  ...
  constructor(options: redis.ConstructorOptions) {
...
export default function init(options: redis.Options = {}): core.StoreInit {
  if (!isPlainObject(options)) {
    throw new TypeError("@cachemap/redis expected options to be a plain object.");
  }

  return (storeOptions: core.StoreOptions) => RedisStore.init({ ...options, ...storeOptions });
}
